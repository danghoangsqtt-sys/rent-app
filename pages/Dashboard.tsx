...
import { 
  ...
  ArrowUpRight, TrendingUp, Users, Zap, Award
} from 'lucide-react';
      ...
      <section className="flex items-center justify-between mt-2">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
             <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-md flex items-center gap-1 ${user?.isPro ? 'bg-amber-500 text-white' : 'bg-slate-200 text-slate-500'}`}>
                {user?.isPro ? <><Award size={10}/> Gói Pro Đã Kích Hoạt</> : <><Zap size={10}/> Tài Khoản Thường</>}
             </span>
          </div>
        </div>
